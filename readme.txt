...
また、もしエンコーディングが統一された差分(パッチ)が出力できたとしても、その出力を流用して一部を切り出し、git applyでパッチを適用しようとすると、元のバイト列と異なっているため、パッチが正しく当たらないことになります。この問題を避けるため、git applyの実行時にdiffとは逆の変換を行う必要があります。
